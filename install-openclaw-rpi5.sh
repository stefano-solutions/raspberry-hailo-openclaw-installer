...
EOF
    ...
    if [[ "$USE_SANITIZER_PROXY_ON_OLLAMA" == "true" ]]; then
        ...
        if [[ ! -f "$PROXY_SRC" ]]; then
            ...
            # Tuning lives as hard-coded constants at the top of the proxy script
            # (no env file, no venv). Local Hailo inference is free; the token caps
            # there only bound latency (~10 tok/s) and curb repetition loops. To
            # change them, edit /usr/local/bin/hailo-sanitize-proxy.py and restart
            # the service.
